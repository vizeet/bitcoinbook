...
def parseHeadersPayload(payload_m: mmap, payloadlen = 0):
    ...
    for i in range(payload['count']):
        hdr = {}
        start = payload_m.tell()
        hdr['blkhash'] = hashlib.sha256(hashlib.sha256(payload_m.read(80)).digest()).digest()[::-1].hex()
...
# we only have genesis block
def createGetAllHeadersPayload():
    version_b = struct.pack('<L', 70013)
    hash_count_b = setVarInt(1)
...
def createGetHeadersPayload(hdr_info_l: list):
    if len(hdr_info_l) == 0:
        payload = createGetAllHeadersPayload()
        return payload
    version_b = struct.pack('<L', 70013)
