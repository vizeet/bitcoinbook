...
def createSignedTransaction(txn_struct: dict, scriptSig_l: list):
    sgntxn_b = b''
    ...
    for i in range(txn_struct['input_count']):
        ...
        scriptSig_b = scriptSig_l[i]
        sgntxn_b += createVarInt(len(scriptSig_b)) + scriptSig_b
        if txn_struct['locktime'] > 0:
            # sequence
...
def getExecutionScript(txn_struct: dict, inp_index: int):
    ...
    if script_type == 'P2SH':
        ...
        prevtxn_b = bytes.fromhex(inp['prevtxn'])[::-1]
        prevtxnindex = inp['prevtxnindex']
        utxo = getUnspentTransactions(prevtxn_b, 
                                prevtxnindex, 
                                chainstate_db_g)
...
def getScriptSigs(txn_struct: dict):
    scriptSig_l = []
    for inp_index in range(txn_struct['input_count']):
        ...
        preimage_b = createSignaturePreimage(txn_struct, 
                                            script_b, 
                                            inp_index)
        print('preimage = %s' % preimage_b.hex())
        inp = txn_struct['inputs'][inp_index]
        if inp['script_type'] == 'P2SH':
            scriptSig_b = b'\x00'
            ...
            pubkeySize_b = encodePushdata(len(pubkey_b))
            scriptSig_b += pubkeySize_b + pubkey_b
        scriptSig_l.append(scriptSig_b)
    return scriptSig_l

def createP2SHTransactionStruct():
    txn = {}
    txn['version'] = 1
    txn['input_count'] = 1
    txn['inputs'] = []
    input0 = {}
    input0['prevtxn'] = \
        'cedf8b5a58c4e2f01414a91927d0c1bea928e895d024bc320909b93129f3f4a0'
...
def createP2PKHTransactionStruct():
    txn = {}
    txn['version'] = 1
    txn['input_count'] = 2
    txn['inputs'] = []
    ...
    input0['privkey'] = 'KwfxnwxpPG1RmhU8jaU8Ron4m1KZGymLAFNaMnSTonoZ7AQfnV53'
    input0['hash_type'] = SIGHASH_ALL
    txn['inputs'].append(input0)
    input1 = {}
    ...
    input1['privkey'] = 'KwfxnwxpPG1RmhU8jaU8Ron4m1KZGymLAFNaMnSTonoZ7AQfnV53'
...
if __name__ == '__main__':
    txn_struct = createP2PKHTransactionStruct()
    scriptSig_l = getScriptSigs(txn_struct)
    signed_txn_b = createSignedTransaction(txn_struct, scriptSig_l)
    print(signed_txn_b.hex())

    txhash_b = bytes.fromhex('f4e7ab82302e4b688f7873ff27292c67897444e983278365fb6fe0d0de5ff151')[::-1]
    print(getUnspentTransactions(txhash_b, 0,chainstate_db_g))
