...
def updateDF(df: DataFrame):
    df['gap_min'] = (df['timestamp'] - df['timestamp'].shift(1)) // 60
    df = df.dropna()
    df = df[df.gap_min != 0]
...
def windowDF(df: DataFrame):
    df['gap_min_avg'] = df['gap_min'].rolling(1000).mean()
#    print(df.to_string())
    return df
...
if __name__ == '__main__':
    ...
    df = updateDF(df)
    df = windowDF(df)
    fig = px.line(df, x = 'date_time', y="gap_min_avg", labels={
                     "date_time": "Date",
                     "gap_min_avg": "Average Block Time"
                 }, title='Bitcoin Average Block Time')
    fig.update_traces(hovertemplate="%{x|%Y/%m/%d} minutes: %{y}")
