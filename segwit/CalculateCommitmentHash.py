...
def getScriptType(tx: dict, in_index: int):
    ...
    print('scriptsig = ', tx_input['scriptsig'])
    ...
    if prevtx_scriptpubkey_b[:3] == b'\x76\xa9\x14' and prevtx_scriptpubkey_b[23:25] == b'\x88\xac' and prevtx_scriptsiglen == 25:
        ...
        print(script_type)
        return script_type
    if prevtx_scriptpubkey_b[:2] == b'\xa9\x14' and prevtx_scriptpubkey_b[22:23] == b'\x87' and prevtx_scriptsiglen == 23:
        if tx['is_segwit'] == True and tx_input['scriptsig'][:6] == '160014' and tx_input['bytes_scriptsig'] == 17:
            script_type = 'P2SH-P2WSH'
            print('witnesses = ', tx_input['witnesses'])
            print(script_type)
            return script_type
        elif tx['is_segwit'] == True and tx_input['scriptsig'][:6] == '220020' and tx_input['bytes_scriptsig'] == 23:
            script_type = 'P2SH-P2WSH'
            print('witnesses = ', tx_inputs['witnesses'])
            print(script_type)
            return script_type
        else:
            script_type = 'P2SH'
...
def getTransactionInfo(txn_m):
    ...
    tx['vbytes'] = (len(wtxid_b) - len(txid_b)) + (len(txid_b) * 4)
    return tx
...
if __name__ == '__main__':
    ...
    commitment_h, cb_tx = calculateCommitmentHash(blk_hb)
